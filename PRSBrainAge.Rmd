...
library(ggplot2); library(ggpubr); library(dplyr);library(sjPlot);library(gtsummary); library(flextable);library(ppcor);library(nlme);library(gridExtra);library(effsize)
...
data <- read.csv('~/Documents/Projects/PRSBrainAgePlasma/data/a4_baseline_plasma/a4_plasma_05012025_full.csv', 
                 row.names=1)  %>%
        dplyr::select(BID, Sex, Baseline_Age, Education, apoe_genotype, Amyloid_Status, PTAU217, AMYLB42, NF.L., GFAP.) %>%
        rename(age = Baseline_Age, YearsEd = Education, Amyloid = Amyloid_Status, pTau217 = PTAU217, AB42 = AMYLB42, NF.L = NF.L., GFAP = GFAP.) %>%
...
data$AB42_n <- ifelse(!is.na(data$AB42), 1, 0)
...
table <- tbl_summary(data,
            include = c(age, Sex, YearsEd, e4, Amyloid, ptau_n, pTau217, AB42_n, AB42, NF.L_n, NF.L, GFAP_n, GFAP),
            missing = "no",
            statistic = list(all_continuous()  ~ c("{mean} ({sd})")),
            label = list(age ~ "Age", YearsEd ~ "Education (Years)", NF.L ~ "NFL (pg/mL)", GFAP ~ "GFAP (ng/mL)", pTau217 ~ "pTau 217 (U/mL)", AB42 ~ "AB42 (pg/mL)"),
...
```{r}
# GM
ab_gm <- glm(AB42 ~ GM_ZSCORE*age, data = data)
ab_gm_standardized <- effectsize::standardize(ab_gm, standardize = "all")
ab_gm_results <- summary(ab_gm_standardized)$coefficients
cat("AB42 GM PRS \n")
cat("Standardized Effect GM PRS * Age: ", ab_gm_results["GM_ZSCORE:age", "Estimate"], 
    "p-val", ab_gm_results["GM_ZSCORE:age", "Pr(>|t|)"], "\n")
cat("Standardized Effect GM PRS: ", ab_gm_results["GM_ZSCORE", "Estimate"], 
    "p-val", ab_gm_results["GM_ZSCORE", "Pr(>|t|)"], "\n")
cat("Standardized Effect Age: ", ab_gm_results["age", "Estimate"], 
    "p-val", ab_gm_results["age", "Pr(>|t|)"], "\n")

# WM
ab_wm <- glm(AB42 ~ WM_ZSCORE*age, data = data)
ab_wm_standardized <- effectsize::standardize(ab_wm, standardize = "all")
ab_wm_results <- summary(ab_wm_standardized)$coefficients
cat("AB42 WM PRS \n")
cat("Standardized Effect WM PRS * Age: ", ab_wm_results["WM_ZSCORE:age", "Estimate"], 
    "p-val", ab_wm_results["WM_ZSCORE:age", "Pr(>|t|)"], "\n")
cat("Standardized Effect WM PRS: ", ab_wm_results["WM_ZSCORE", "Estimate"], 
    "p-val", ab_wm_results["WM_ZSCORE", "Pr(>|t|)"], "\n")
cat("Standardized Effect Age: ", ab_wm_results["age", "Estimate"], 
    "p-val", ab_wm_results["age", "Pr(>|t|)"], "\n")

# FC
ab_fc <- glm(AB42 ~ FC_ZSCORE*age, data = data)
ab_fc_standardized <- effectsize::standardize(ab_fc, standardize = "all")
ab_fc_results <- summary(ab_fc_standardized)$coefficients
cat("AB42 FC PRS \n")
cat("Standardized Effect FC PRS * Age: ", ab_fc_results["FC_ZSCORE:age", "Estimate"], 
    "p-val", ab_fc_results["FC_ZSCORE:age", "Pr(>|t|)"], "\n")
cat("Standardized Effect FC PRS: ", ab_fc_results["FC_ZSCORE", "Estimate"], 
    "p-val", ab_fc_results["FC_ZSCORE", "Pr(>|t|)"], "\n")
cat("Standardized Effect Age: ", ab_fc_results["age", "Estimate"], 
    "p-val", ab_fc_results["age", "Pr(>|t|)"], "\n")
```
...
# Sensitivity analysis adding other PRS, covariates and interactions
pTau217_biomakers <- glm(pTau217 ~ GM_ZSCORE*age + WM_ZSCORE*age + FC_ZSCORE*age + Sex + YearsEd + e4 + Amyloid + GFAP + NF.L + AB42, data = data)
...
cat("Adding PRS, covariates and interactions \n")
...
p_ptau_gm <- plot_model(model_ptau_gm, type = c('pred'), terms = c('age', 'GM_ZSCORE [-1, 0, 1]'), show.values = TRUE)
p_ptau_gm
```
